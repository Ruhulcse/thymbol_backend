...
const saveCategoryData = async (req, res) => {
  ...
  const { category_code, category_name, description, operational_group } =
    sencondParseData;
  ...
  try {
    ...
    if (categoryLogo) {
      returnUpdateCategory = await updateCategoryLogo(
        savedCategory._id,
        category_name,
        categoryLogo
      );
    ...
    res.status(501).send({
      ...
      data: [],
      statusCode: 501,
      message: "Error Found! Can not create category!",
    });
  }
};
...
const editCategory = asyncHandler(async (req, res) => {
  ...
  if (mongoose.Types.ObjectId.isValid(req.params.id)) {
    ...
    if (categoryLogo) {
      ...
      returnUpdateCategory = await updateCategoryLogo(
        req.params.id,
        sencondParseData.category_name,
        categoryLogo
      );
    }
...
const getAllCategories = asyncHandler(async (req, res) => {
  const categoryData = await Category.find().select(
    "-operational_group -category_code"
  );
