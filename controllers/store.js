...
const saveStoreData = async (req, res) => {
    ...
    const { store_name, category, address, website_link, social_media_link, business_hours, location } = sencondParseData;
    ...
    try {
        ...
        if (storeLogo) {
            const updatedLogoDetails = await updateStoreLogo(savedStore._id, storeLogo);
            returnUpdateStore = updatedLogoDetails;
        }
        if (storeDocuments) {
            const updateStroeWithDoc = await updateStoreDocuments(savedStore._id, storeDocuments);
        ...
        console.error(err);
        res.status(501).send({ error: true, data: [], statusCode: 501, message: "Error Found! Can not create store!" });
    }
};
    ...
    try {
        if (Array.isArray(documentsFile)) {
            for (let i = 0; i <= documentsFile.length; i++) {
                ...
                const extname = filetypes.test(path.extname(singleFile.name).toLowerCase());
...
const deleteStore = asyncHandler(async (req, res) => {
    ...
    if (store) {
        ...
        res.status(200).send({ message: "Store deleted successfully!", error: false, statusCode: 200 });
    } else {
        res.status(404).send({ message: "Data with given ID not found!", error: true, statusCode: 404 });
    }
});
...
const getAllCategoryForDropDown = asyncHandler(async (req, res) => {
    ...
    if (categories) {
        res.status(200).send({ message: "Successfully Retrive Category Info!", error: false, data: categories });
    } else {
        res.status(404).send({ message: "Categories not found!", error: true, data: [] });
    }
...
const getSubCategoriesWithParentCatId = asyncHandler(async (req, res)=>{
    ...
    if (subCategories) {
        res.status(200).send({ message: "Successfully Retrive Sub Category Info!", error: false, data: subCategories });
    } else {
        res.status(404).send({ message: "Categories not found!", error: true, data: [] });
    }
})
